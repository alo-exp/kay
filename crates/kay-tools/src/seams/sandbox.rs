...
impl Sandbox for NoOpSandbox {
    async fn check_shell(&self, _c: &str, _w: &Path) -> Result<(), SandboxDenial> {
        Ok(())
    }
    async fn check_fs_read(&self, _p: &Path) -> Result<(), SandboxDenial> {
        Ok(())
    }
    async fn check_fs_write(&self, _p: &Path) -> Result<(), SandboxDenial> {
        Ok(())
    }
    async fn check_net(&self, url: &Url) -> Result<(), SandboxDenial> {
...
mod tests {
    ...
    async fn noop_blocks_file_url_scheme() {
        ...
        assert!(
            res.is_err(),
            "file:// must be rejected by Phase 3 NoOpSandbox"
        );
        if let Err(d) = res {
