//! fs_search — parity delegation to `ServicesHandle::fs_search`.

use async_trait::async_trait;
use forge_domain::{FSSearch, ToolName, ToolOutput};
...
impl FsSearchTool {
    pub fn new() -> Self {
        let name = ToolName::new("fs_search");
        let description = "Search files matching a regex. Results capped by \
            max_search_lines and max_search_result_bytes."
            .to_string();
        let mut schema = serde_json::to_value(schemars::schema_for!(FSSearch))
            .unwrap_or_else(|_| serde_json::json!({ "type": "object" }));
        harden_tool_schema(
            &mut schema,
            &TruncationHints {
                output_truncation_note: Some(
                    "Results capped by max_search_lines and max_search_result_bytes.".to_string(),
                ),
            },
        );
...
mod tests {
    ...
    #[test]
    fn name_is_fs_search() {
        let t = FsSearchTool::new();
        assert_eq!(t.name().as_str(), "fs_search");
    }
}
