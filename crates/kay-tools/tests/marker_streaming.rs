...
// The Unix variant uses `printf` (shell builtin) and `/tmp` (Unix cwd).
// Windows gets an equivalent below that uses PowerShell's `Write-Output`.
#[cfg(not(windows))]
#[tokio::test(flavor = "multi_thread")]
...
/// Windows equivalent of `streams_multiple_lines_in_order`. Uses
/// PowerShell-native `Write-Output` with three separate statements and
/// `std::env::temp_dir()` for the cwd (since `/tmp` is Unix-only).
/// Verifies SHELL-01 + SHELL-03 invariants on PowerShell pipelines.
#[cfg(windows)]
#[tokio::test(flavor = "multi_thread")]
async fn streams_multiple_lines_in_order() {
    let tool = ExecuteCommandsTool::with_timeout(std::env::temp_dir(), Duration::from_secs(10));
    ...
    // Three Write-Output calls in sequence — each emits its own line. Unlike
    // `printf 'a\nb\nc\n'` (bash-builtin), `printf` is not a PowerShell
    // cmdlet, so we use the native equivalent.
    let _out = tool
        .invoke(
            json!({"command": "Write-Output a; Write-Output b; Write-Output c"}),
            &ctx,
            "call_2",
        )
        .await
