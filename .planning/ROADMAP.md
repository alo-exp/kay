...
- [x] **Phase 10: Multi-Session Manager + Project Settings** *(COMPLETE 2026-05-04 — shipped as v0.5.0; PR #20 squash-merged)* *(planned 2026-04-24)* - Spawn/pause/resume/fork sessions from GUI and TUI, project workspace + keyring binding, model allowlist picker, command-approval dialog, settings panel.
- [x] **Phase 11: Cross-Platform Hardening + Release Pipeline** *(COMPLETE 2026-05-04 — shipped as v0.5.0; AppImage deferred to v0.6.0)* - Signed + notarized bundles for macOS (arm64/x64), Windows (x64), Linux (x64/arm64); `cargo install kay` (standalone CLI); `cargo install kay-tui` (TUI); minisign updater for desktop bundle.
